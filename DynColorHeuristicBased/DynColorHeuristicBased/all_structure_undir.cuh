 ...
 */
void readin_changes(char* myfile, vector<changeEdge>& allChange, vector<changeEdge>& allChange_Del, vector<ColList>& AdjList, int& totalInsertion, int* vertexcolor, int* vertex_prop)
{
	...
	while (fgets(line, 128, graph_file) != NULL)
	{
		...
		if (inst_status == 1)
		{
			...
			//Heuristic based prop//using LDF here. so storing degree
			vertex_prop[n1] += 1;
			vertex_prop[n2] += 1;
...
*/
void read_graphEdges(vector<ColList>& AdjList, char* myfile, int* vertex_prop)
{
	...
	while (fgets(line, 128, graph_file) != NULL)
	{
		...
		AdjList.at(n2).push_back(c1);
		AdjList.at(n1).push_back(c2);
		//Heuristice dependent operation//considering LDF and storing degree
		vertex_prop[n1] += 1;
		vertex_prop[n2] += 1;
	}
	fclose(graph_file);
