...
void write_parts(char* filename, int num_verts, int* parts)
{
  ...
  //Addition by AK
  string parfileAll = "partitionAll.txt";
  char* parfileAllName = new char[parfileAll.length() + 1];
  strcpy(parfileAllName, parfileAll.c_str());
  ofstream outfileAll;
  outfileAll.open(parfileAllName);
  for (int i = 0; i < num_verts; i++)
  {
	  string parfile = "partition" + to_string(parts[i]) + ".txt";
	  char* parfileName = new char[parfile.length() + 1];
	  strcpy(parfileName, parfile.c_str());
	  ofstream outfile;
	  outfile.open(parfile, std::ios_base::app); // append instead of overwrite
	  outfile << i << endl;
	  outfile.close();
	  outfileAll << parts[i] << endl; //storing all partID in one file here
  }  
  outfileAll.close();
}
