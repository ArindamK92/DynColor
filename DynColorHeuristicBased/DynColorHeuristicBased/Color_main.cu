...
*****changed edges should be unique*****
*/
int main(int argc, char* argv[]) {
	...
	vector<changeEdge> allChange, allChange_Del;
	changeEdge* allChange_device; //stores all change edges marked for insertion in GPU
	...
	////Store prop of vertex required for Heuristic based ordering
	cudaStatus = cudaMallocManaged(&vertex_prop, nodes * sizeof(int));
	if (cudaStatus != cudaSuccess) {
		fprintf(stderr, "cudaMalloc failed at vertex_prop structure");
	}
	...
	read_graphEdges(AdjList, argv[1], vertex_prop);
	...
	readin_changes(argv[5], allChange, allChange_Del, AdjList, totalInsertion, vertexcolor, vertex_prop);
	...
	//test//print vertex_prop
	cout << "printing prop::\n";
	for (int i = 0; i < nodes; i++)
	{
		cout << i << " prop: " << vertex_prop[i]<<endl;
	}
	...
	transfer_data_to_GPU(AdjList, AdjListTracker, AdjListFull, AdjListFull_device,
		nodes, edges, totalInsertion, AdjListTracker_device, 
		allChange, allChange_device, totalChangeEdges,
	...
	auto startTimeChangeEdge = high_resolution_clock::now(); //Time calculation start
	processChangeEdge << < numberOfBlocks, THREADS_PER_BLOCK >> > (allChange_device, vertexcolor, totalChangeEdges, AdjListFull_device, AdjListTracker_device, affected_marked, change, vertex_prop);
	cudaDeviceSynchronize(); //comment this if required
	auto stopTimeChangeEdge = high_resolution_clock::now();//Time calculation ends
	auto durationChangeEdge = duration_cast<microseconds>(stopTimeChangeEdge - startTimeChangeEdge);// duration calculation
	cout << "**Time taken for processing del edges: "
		<< float(durationChangeEdge.count()) / 1000 << " milliseconds**" << endl;
	total_time += float(durationChangeEdge.count()) / 1000;
	...
	//	insEdge << < numberOfBlocks, THREADS_PER_BLOCK >> > (allChange_Ins_device, vertexcolor, previosVertexcolor, totalChangeEdges, AdjListFull_device, AdjListTracker_device, affected_marked, change);
	//	cudaDeviceSynchronize(); //comment this if required
	...
	*counter = cuCompactor::compact<int, int>(affected_marked, affectedNodeList, nodes, predicate(), THREADS_PER_BLOCK);
	...
	while (*change > 0)
	{
		*change = 0;
		...
		findEligibleNeighbors << < numberOfBlocks, THREADS_PER_BLOCK >> > (affectedNodeList, AdjListFull_device, AdjListTracker_device, affected_marked, vertexcolor, counter, vertex_prop);
		...
		*counter = cuCompactor::compact<int, int>(affected_marked, affectedNodeList, nodes, predicate(), THREADS_PER_BLOCK);
		...
		recolorNeighbor << < numberOfBlocks, THREADS_PER_BLOCK >> > (affectedNodeList, vertexcolor, AdjListFull_device, AdjListTracker_device, affected_marked, counter, change, vertex_prop);
		cudaDeviceSynchronize();
	}
