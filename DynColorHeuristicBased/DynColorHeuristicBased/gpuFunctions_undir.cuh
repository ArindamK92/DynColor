...
/*
* finds if relative ordering node1 '<' node2 is true
*/
__device__ bool isPrior_RelativeOrdering(int* vertex_prop, int node_1, int node_2) {
	
	//Here we use Largest Degree First//we need to change this depending on heuristic
	int prior = vertex_prop[node_1] > vertex_prop[node_2] ? true : false;
	if (vertex_prop[node_1] == vertex_prop[node_2])
	{
		prior = node_1 < node_2 ? true : false;
	}
	return prior;
}

/*
* findMinAvailableColor fn finds the min available color
*/
__device__ int findMinAvailableColor(int node, ColWt* AdjListFull_device, int* AdjListTracker_device, int* vertex_prop, int* vertexcolor) {

	int saturationColorPlusMask = 0;
	for (int j = AdjListTracker_device[node]; j < AdjListTracker_device[node + 1]; j++) {
		int neighbor = AdjListFull_device[j].col;
		if (AdjListFull_device[j].flag != -1 && isPrior_RelativeOrdering(vertex_prop, neighbor, node)) //if n'<'v then only n.color will affect v.color
		{
			saturationColorPlusMask = saturationColorPlusMask | int(exp2(double(vertexcolor[neighbor])));
		}
	}
	return FirstZeroBit(saturationColorPlusMask);
}
...
* 
*/
__global__ void processChangeEdge(changeEdge* allChange_device, int* vertexcolor, int totalChangeEdges, ColWt* AdjListFull_device, int* AdjListTracker_device, int* affected_marked, int* change, int* vertex_prop) {

	for (int index = blockIdx.x * blockDim.x + threadIdx.x; index < totalChangeEdges; index += blockDim.x * gridDim.x)
	{
		//for e(node1,node2) check both node1 and node2
		int node = allChange_device[index].node1;
		int smallest_available_color = findMinAvailableColor(node, AdjListFull_device, AdjListTracker_device, vertex_prop, vertexcolor);
		if (smallest_available_color != vertexcolor[node])
		{
			//previosVertexcolor[node] = vertexcolor[node];
			vertexcolor[node] = smallest_available_color;
			affected_marked[node] = 1;
			*change = 1;
			//printf("color of %d became:%d", node, vertexcolor[node]);
		}

		node = allChange_device[index].node2;
		smallest_available_color = findMinAvailableColor(node, AdjListFull_device, AdjListTracker_device, vertex_prop, vertexcolor);
		if (smallest_available_color != vertexcolor[node])
		{
			//previosVertexcolor[node] = vertexcolor[node];
			vertexcolor[node] = smallest_available_color;
...
*/
__global__ void findEligibleNeighbors(int* affectedNodeList, ColWt* AdjListFull_device, int* AdjListTracker_device, int* affected_marked, int* vertexcolor, int* counter, int* vertex_prop) {
	for (int index = blockIdx.x * blockDim.x + threadIdx.x; index < *counter; index += blockDim.x * gridDim.x)
	{
		...
		for (int j = AdjListTracker_device[node]; j < AdjListTracker_device[node + 1]; j++)
		{
			...
			if (AdjListFull_device[j].flag > -1 && vertexcolor[node] == vertexcolor[neighbor]) //node < AdjListFull_device[j].col is used to select which vertex color to update in confusion
			{
				int candidate = isPrior_RelativeOrdering(vertex_prop, node, neighbor) == true ? neighbor : node; //if v'<'n then candidate n
				//atomicAdd(&AdjListFull_device[j].flag, -3);
				affected_marked[candidate] = 1;
				//printf("AdjListFull_device[j].flag of %d = %d\n", AdjListFull_device[j].col, AdjListFull_device[j].flag);
			}
			//mark neighbor if v '<' n
			if (AdjListFull_device[j].flag > -1 && isPrior_RelativeOrdering(vertex_prop, node, neighbor))
			{
				affected_marked[neighbor] = 1;
			}
...
*/
__global__ void recolorNeighbor(int* affectedNodeList, int* vertexcolor, ColWt* AdjListFull_device, int* AdjListTracker_device, int* affected_marked, int* counter, int* change, int* vertex_prop) {
